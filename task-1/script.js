...
function dataLoaded(err,rows){
    ...
    .timeRange([new Date(2011,6,15),new Date(2013,11,15)])
    .binSize(d3.time.week);

   /* d3.select('.plot')
      .datum(rows)
      .call(timeSeries1);  // going to run export function in plot div, which write in html*/
      var tripsByStation = d3.nest() // group by startStation
